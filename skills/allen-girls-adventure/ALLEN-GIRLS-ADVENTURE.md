---
name: allen-girls-adventure
description: Guided SOP for setting up and using Allen Girls Adventure from OpenClaw. Use when the user wants to clone the Allen Girls Adventure repo, choose a startup mode, configure recommended API keys, start the service, or generate a classroom from requirements or a PDF. Run one phase at a time and ask for confirmation before each state-changing step.
user-invocable: true
metadata: { "openclaw": { "emoji": "🏫" } }
---

# Allen Girls Adventure Skill
...
- Do not assume the OpenClaw agent's own model or API key will be reused by Allen Girls Adventure.
- Allen Girls Adventure classroom generation uses Allen Girls Adventure server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Allen Girls Adventure server-side config files may control provider selection and defaults.
  ...
  "skills": {
    "entries": {
      "allen-girls-adventure": {
        ...
        "config": {
          "accessCode": "sk-xxx",
          "repoDir": "/path/to/Allen Girls Adventure",
...
If no `accessCode` in config, ask the user how they want to use Allen Girls Adventure:

1. **Use hosted Allen Girls Adventure** (recommended for quick start) — Requires an access code from open.maic.chat. No local setup needed.
...
Use this when the user has not installed Allen Girls Adventure yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Allen Girls Adventure

After the user has chosen a startup mode and configured keys, start Allen Girls Adventure using the chosen method, then verify the service with `GET {url}/api/health`.
