...
const launchPayloadSchema = z.object({
  learnerId: uuidOrNull,
  guestSessionId: uuidOrNull,
  ...
  resumePlaybackCompleted: z.boolean().nullable().optional(),
...
export function decodeAndVerifyAgaLaunch(
  ...
  if (!hasAgaUserIdentity(parsed.data)) {
    log.warn('AGA launch missing learnerId and guestSessionId');
    return { ok: false, error: 'missing_user_identity' };
  }

  const identity = normalizeAgaLaunchUserIds(parsed.data)!;
  return {
    ok: true,
    launch: {
      ...parsed.data,
